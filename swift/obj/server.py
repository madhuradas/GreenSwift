...
class ObjectController(object):
	...
	List.remove('')
	f.close()
	self.logger.info(request.headers.get('X-Container-Host', None))
        try:
            device, partition, account, container, obj = \
                split_path(unquote(request.path), 5, 5, True)
            validate_device_partition(device, partition)
		...
		if device == i:
		    if partition in dict_info.keys():
			dict_info[partition].append(device)
		    else:
			dict_info[partition] = [device]
		    device = "ssd"
